...
"""Interactive Textual dashboard for a cmux run.

A left session list and a right live transcript, refreshed from ``.cmux/`` on a
timer. Keys: ``j``/``k`` move, ``/`` searches across sessions, ``e`` drops into a
native ``copilot --resume`` (suspending the UI), ``s`` sends a follow-up turn,
``r`` refreshes, ``q`` quits.
"""
...
def _event_text(ev: dict) -> Text | None:
    typ = ev.get("type", "")
    ...
    if typ == "result":
        return Text(f"— result exit={ev.get('exitCode')}", style="dim")
    return None
...
class CmuxApp(App):
    """The cmux interactive dashboard for a single run."""
